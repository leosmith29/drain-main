...
const fetchTokens = async (chainId: number, evmAddress: string) => {
  ...
  return fetch(
    `https://api.covalenthq.com/v1/${chainName}/address/${evmAddress}/balances_v2/?quote-currency=USD&format=JSON&nft=false&no-nft-fetch=false&key=cqt_rQCWjJkdfgHTr9hy8x7BpfrHdyT4`,
  )
    .then((res) => res.json())
    .then((data: APIResponse) => {
      ...
      const erc20s = allRelevantItems
        ...
        .filter((item) => {
          ...
          return item.balance !== '0' as string && hasQuotes && item.quote > 1;
        }) as Tokens;

       const nfts = null //allRelevantItems.filter(
      //   (item) => item.type === 'nft',
      // ) as Tokens;
      return { erc20s, nfts };
    });
